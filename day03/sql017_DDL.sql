...
/*
    ...
    -- 타입 종류
    INT, BIGINT(long과 비슷), BINARY(n)[이진데이터], BIT[0|1]
    CHAR(n)[고정문자열], VARCHAR(n)[가변문자열], NCHAR(n), NVARCHAR(n), N-[유니코드],
    예(username : CHAR(10)) 'name'
    CHAR(10) = '이름        ' 이름을 넣고 10개의 자리 중 빈곳은 전부 스페이스 입력 ' '
    VARCHAR(10) = '이름' 이름을 넣고 10개의 자리 중 빈곳은 전부 없앰
    DATA[날짜], DATETIME[날짜시간], DECIMAL(18, 0)[소수점표현실수], FLOAT[실수],
    IMAGE[이미지바이너리], SMALLINT[255까지의 정수], TEXT[2GB까지의 텍스트], NTEXT[유니코드 2GB]
...
-- 기본키를 통합
CREATE TABLE NewBook (
    bookid INT PRIMARY KEY, -- 기본키로 bookid 지정
    bookname VARCHAR(20),
    publisher VARCHAR(20),
    price INT
);
...
CREATE TABLE NewBook (
    ...
    publisher VARCHAR(20) UNIQUE, -- UNIQUE 제약조건
    price INT DEFAULT 10000 CHECK(price > 1000) -- 기본값 제약조건-CHECK 제약조건 : 조건을 만족하지 않으면 입력이 불가능
